...
export function ConnectHero({
  ...
  return (
    <div className={`hero flex flex-col gap-4 ${running ? "" : "hero-stopped"}`}>
      <div className="relative z-[1] flex items-center justify-between gap-3">
        ...
        {status && (
          <span className="hero-addr rounded-full px-3 py-1 font-mono text-xs">
            {displayHost} : {port}
          </span>
        )}
      </div>
      ...
      <div className="relative z-[1] grid grid-cols-3 gap-2.5">
        <HeroMetric icon="activity" label="总请求数" value={status?.stats.totalRequests ?? 0} />
        <HeroMetric icon="alert" label="错误数" value={status?.stats.totalErrors ?? 0} />
        <HeroMetric
          icon="clock"
          label="运行时间"
...
function HeroMetric({
  ...
  return (
    <div className="hero-metric">
      <div className="text-[23px] font-extrabold leading-tight tracking-tight">{value}</div>
