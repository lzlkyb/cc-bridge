...
// 第 2 批变动：`std::path::PathBuf` 与 `similar::TextDiff` 随备份域一同搬走了。
// PathBuf 仅剩测试模块在用（那里自带 use），TextDiff 已无人用——不删会被
// `clippy -D warnings` 直接卡住（本批编译就是先报这两条）。
...
// 第 3 批变动：`Ordering` / `Duration` / `Instant` / `TcpStream` / `timeout` / `crate::backup`
// 六个 import 全随 status 域搬进了 commands/status.rs —— get_status 是它们在本文件里的
// 唯一用户（轮询计时、TCP 探活、备份统计）。不删同样会被 `clippy -D warnings` 卡住。
use crate::audit;
use crate::browse;
...
// 第 2 批：备份域（含安全函数 assert_backup_path_in_scope）拆到 commands/backup.rs。
// 第 3 批：状态遥测与防火墙拆出（firewall_cmds 避开 crate::firewall 重名）。
mod firewall_cmds;
mod status;
