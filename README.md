...
| 版本 | 变更 |
|---|---|
| v2.7.6 | 终端底部多了一条状态栏（当前目录 / 退出码 / git 分支，4 套预设）；粘贴多行不再弹框——内容整块进命令行、显示成多行，按回车才执行；拖文件到终端直接落到远端当前目录，不再每次问；设置页加了左侧导航。 |
| v2.7.5 | 检查更新和下载更新包现在走同一个镜像了。之前读更新信息用的是新地址，实际下载却还走那个证书过期过的旧镜像——两边不统一时，可能出现「提示有新版本，却怎么也下不下来」。 |
| v2.7.4 | 终端里的中文不再随机烂字（以前每约 4KB 就烂一个）；刷大量输出时不再把界面卡死；同一台机器现在可以开多个终端，一边 `tail -f` 一边敲命令；重连不再清空之前的输出。 |
| v2.7.3 | v2.7.3 版本更新 |
| v2.7.2 | v2.7.2 版本更新 |
| v2.7.1 | SSH 终端现在能粘贴了（Ctrl+V / 右键 / 工具栏）；文件传输加上了进度条和取消按钮，覆盖前会先问一句；并修复了带空格的文件名传不了的问题。 |
| v2.7.0 | v2.7.0 版本更新 |
| v2.6.4 | v2.6.4 版本更新 |
...
| v2.6.1 | 修复 **mac 版发不出来**（v2.6.0 的 mac 构建失败，Windows 不受影响）。新增：导入外挂 MCP 工具时可以「运行一下」，就地看到它提供哪些工具（名字 + 一句话）；设置页服务行与启用前的风险确认框也会列出同一份清单，跑一次即可 |
| v2.6.0 | **外挂 MCP 桥**：把本机已装的任意 MCP 工具（代码索引、文档检索等）转给远程 Claude Code 用。可从 Claude Code / Cursor / Claude Desktop 的现有配置一键导入，逐个启用（总开关默认关、启用前弹风险确认并逐字列出它要启动的命令）；支持多个项目（远程可按调用指定工作目录，需单独开启）。协议层通用，新接一个工具不需要改代码。另修掉国内加速镜像失败时静默报成功（已影响近四个版本的国内下载与更新） |
| v2.5.0 | **连接页整页改版**：服务状态改为链路展示（本机服务 → 防火墙 → 远程调用，能告诉你卡在哪一环），新增四张带趋势小图的指标卡与健康度 / 安全治理卡，各卡可点直达对应设置，并补齐了一批微动画。另修掉 macOS 最小化到 Dock 后后台刷新不停、以及后台命令很长时撑破窗口两个问题 |
| v2.4.2 | **macOS 专项修复**：修掉 mac 上 CPU 长期占满一个核心（mac 版自首发即存在；本机地址监听任务空转且每轮枚举网卡），并恢复 mac 上地址变化的托盘提示与桌面通知；Windows 不受影响。内部：`commands.rs` 拆分完成（2700 行 → 9 个子模块 + 48 行聚合，四批增量，新增 `tools/fingerprint.py` 做纯搬动等价性校验） |
...
| v2.3.16 | 新增桌面通知能力（AI 主动提醒 + 后台命令完成自动通知）与命令安全策略；补充前端单测 |
| v2.3.15 | v2.3.15 版本更新 |
| v2.3.14 | v2.3.14 版本更新 |
| v2.3.13 | **修掉发布版执行命令弹黑框 + 后台命令与关于卡片体验优化**：执行 cmd/bash 命令不再弹黑色控制台窗口；没有新更新时关于卡片不再自动展开；后台命令列表整行可点即可查看输出；write_files 覆盖文件自动保留原编码（中文文件不再被静默转码） |
| v2.3.12 | **构建修复**：修复 v2.3.11 CI 构建中 tauri 命令找不到导致安装包未能生成的问题（发版前同步 package.json 版本号、预构建脚本与主构建流程分离）；用户侧内容与 v2.3.11 一致（红点引导 + 接入模式折叠 + 更新弹框分流 + 托盘闪烁修复等） |
| v2.3.11 | **设置红点引导 + 接入模式折叠 + 更新弹框分流 + 托盘闪烁修复**：设置页红点不再每次打开都乱亮，且能点击引导展开「更新历史」、看完才标已读；自动检查更新不再弹「已是最新」框，仅手动点「检查更新」才提示；连接页「接入模式」收进手风琴折叠区、首次接入默认全展开引导；IP 变化给出 banner 与倒计时；修掉托盘图标闪烁 |
| v2.3.10 | **托盘复制 IP 命令跟随作用域 + 切换联动修复**：托盘右键「复制 IP 命令」现在与连接页选的作用域一致——选「项目级」复制针对项目 .mcp.json 的命令（设了项目路径时自动带 `cd` 前缀），选「全局模式」复制针对用户级 ~/.claude.json 的命令，与连接页「连接命令」逐字对应；修掉切换作用域后托盘命令不跟着变的遗留问题 |
| v2.3.9 | **新手引导修复 + 后端健壮性加固 + MCP 工具三项增强**：设置页新增「重新查看使用引导」入口、修复新手引导交互缺陷；全量审计后一批后端健壮性修复（非法正则/异常 ipynb 不再 panic、备份保留数为 0 不再误删、配置写入事务回滚等）；edit_files 在 old_string 多带首尾空白时主动提醒；read_files 加固二进制识别（PNG/EXE 经 GBK 回退误判为文本的洞补上）；search_files 命中过多时明确告知结果已截断 |
| v2.3.8 | **SSE 流式传输 + 连接页手风琴重构 + bash 探测兜底**：设置页一键切换 HTTP/SSE 传输，连接命令与托盘复制自动联动，切 SSE 时弹现成远端迁移命令；连接页改为手风琴（首次自动展开、之后收起、一次只开一个、自动滚动 + 平滑动画）；版本历史弹框支持一键全屏、长内容自动换行；修复 bash 探测误报与日志搜索框聚焦内边框；移除传输安全常驻提醒 |
| v2.3.7 | **性能提速 + 导入配置修复 + 更新弹框升级路径**：审计日志/白名单/网卡枚举加缓存（空闲近乎零开销）；日志页渲染隔离更跟手；修复导入配置后新目录未立即生效；更新弹框清晰展示「当前版本 → 新版本」 |
| v2.3.6 | **bash 壳层切换 + 托盘复制修复 + 二进制防乱码 + 托盘 IP 替换**：设置页新增 cmd/bash 命令壳层切换（bash 带远端感知+不可用前端拦截）；托盘「复制连接命令」改为系统级剪贴板+通知（不再因窗口隐藏失败）；read_files 新增二进制守卫防止 PNG/EXE 被误判为文本返回乱码；托盘新增「复制 IP 替换命令」项（sed 命令，一键复制）；edit_files 匹配失败时检测首尾多余空白并给出提示 |
